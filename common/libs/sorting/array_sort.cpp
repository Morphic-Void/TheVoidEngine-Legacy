...
bool ArraySort::init( IArrayQuery& query, const unsigned int count )
{
    kill();
...
{   //  resort a single key : required if the indexed data key has been modified
    if( contains( index ) )
    {
        const void* key = m_query->get( index );
        if( key != NULL )
        {
            if( unlink( index ) )
            {
    ...
    if( valid() )
    {
        ...
        if( count > 1 )
        {
            ...
            while( count )
            {
                ...
                node.balance = 0;
                if( !insert( index, m_query->get( index ) ) )
                {
                    return( false );
                }
                --count;
...
int ArraySort::find( const void* const key ) const
{
    ...
    if( valid() && ( key != NULL ) )
    {
        ...
        while( found >= 0 )
        {
            int compare = m_query->cmp( key, found );
            if( compare == 0 ) break;
...
bool ArraySort::insert( const int index, const void* const key )
{
    if( index >= 0 )
    {
        ...
        {   //  this is not the first node to be inserted so will need sorting
            for(;;)
            {
                int compare = m_query->cmp( key, parent );
                if( compare == 0 )
...
{
    if( static_cast< unsigned int >( index ) >= static_cast< unsigned int >( m_nodeCount ) )
    {
        return( false );
    }
    Node& node = m_nodes[ index ];
    ...
    if( prevIndex >= 0 )
    {
        ...
        if( m_query->cmp( key, prevIndex ) <= 0 )
        {
            return( false );
        }
    }
    ...
    if( nextIndex >= 0 )
    {
        ...
        if( m_query->cmp( key, nextIndex ) >= 0 )
        {
            return( false );
        }
    }
