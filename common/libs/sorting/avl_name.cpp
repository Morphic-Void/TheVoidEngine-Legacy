...
Node* Node::find( const uint8_t* name ) const
{
	if( !this ) return( 0 );
	const Node* search = top();
	while( search )
	{
		int compare = cmp( name, search->m_name );
		if( compare == 0 ) break;
...
Node* Node::rename( const uint8_t* const name )
{
	if( !this ) return( 0 );
...
Node* Node::insert( Node* const root )
{
	...
	if( !loose() ) return( 0 );
	if( !root ) return( this );
	if( root->find( m_name ) != 0 ) return( 0 );
	m_balance = 0;
	Node* parent = root->top();
	Node* child = this;
	for(;;)
	{
		int compare = cmp( m_name, parent->m_name );
		if( compare == 0 )
		{
			parent = child = 0;
			break;
...
int Node::cmp( const uint8_t* const name1, const uint8_t* const name2 )
{
	int char1 = 0;
	int char2 = 0;
    int index = 0;
	do
	{
		char1 = static_cast< int >( name1[ index ] );
		char2 = static_cast< int >( name2[ index ] );
		char1 -= char2;
		++index;
	}
...
bool Tree::rename( Node* const node, const uint8_t* const name )
{
	if( this && node && name )
	{
		if( node->top() == m_root )
		{
			Node* root = node->rename( name );
			if( root )
			{
				m_root = root;
				return( true );
...
Node* Tree::find( const uint8_t* const name ) const
{
	return( ( this && name ) ? m_root->find( name ) : 0 );
}
