...
Node* Node::find( const Blob& blob ) const
{
	if( !this ) return( 0 );
	const Node* search = top();
	while( search )
	{
		intptr_t compare = cmp( blob, search->m_blob );
		if( compare == 0 ) break;
		int index = ( ( compare >> ( ( sizeof( intptr_t ) << 3 ) - 1 ) ) + 1 );
...
Node* Node::rename( const Blob& blob )
{
	if( !this ) return( 0 );
	Node* found = find( blob );
	if( found )
	{
...
Node* Node::insert( Node* const root )
{
	...
	if( !loose() ) return( 0 );
	if( !root ) return( this );
	if( root->find( m_blob ) != 0 ) return( 0 );
	m_balance = 0;
	Node* parent = root->top();
	Node* child = this;
	for(;;)
	{
		intptr_t compare = cmp( m_blob, parent->m_blob );
		if( compare == 0 )
		{
			parent = child = 0;
			break;
...
intptr_t Node::cmp( const Blob& blob1, const Blob& blob2 )
{
    intptr_t delta = ( blob1.size - blob2.size );
    if( delta == 0 )
    {
        delta = static_cast< intptr_t >( memcmp( reinterpret_cast< const void* const >( blob1.data ), reinterpret_cast< const void* const >( blob2.data ), blob2.size ) );
    }
    return( delta );
}
...
bool Tree::rename( Node* const node, const Blob& blob )
{
	if( this && node && ( blob.data || !blob.size ) )
	{
		if( node->top() == m_root )
		{
			Node* root = node->rename( blob );
			if( root )
			{
				m_root = root;
				return( true );
...
Node* Tree::find( const Blob& blob ) const
{
	return( ( this && ( blob.data || !blob.size ) ) ? m_root->find( blob ) : 0 );
}
